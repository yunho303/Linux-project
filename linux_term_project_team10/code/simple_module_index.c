...
struct my_list_head {
	struct my_list_head  *next_n, *prev_n, *next_i, *prev_i;
};
...
static inline void list_insert_by_value_init(struct my_list *pos, struct my_list_head *head, int value, int new_value) {
	...
	if (new_value%2 == 1 && head->prev_n==head) {
		if (head->next_n == head) {
			...
			head->next_n->next_i = new;
			new->prev_i = head->next_n;
			new->next_i = NULL;
		}
		head->prev_n = new;
		new->next_n = head;
	...
	if (new_value%2 == 0 && head->next_n==head){
		if (head->prev_n == head) {
			...
			head->prev_n->next_i = new;
			new->prev_i = head->prev_n;
			new->next_i = NULL;
		}
		head->next_n = new;
		new->prev_n = head;
	...
	if (value%2 == 1) {
		list_for_each_entry_left(pos, head, list){
			if (pos->num == value) {
				...
				//delete from i
				struct my_list_head *pos_original_next_i = pos->list.next_i;
				pos->list.prev_i->next_i = pos_original_next_i;
				if (pos_original_next_i !=NULL) {
					pos_original_next_i->prev_i = pos->list.prev_i;
				}				
				kfree(pos);
				break;
			}
...
void test(void){
	...
	int i = 0;
	for (i=2; i<4; i++){
		list_insert_by_value_init(mylist, &head, i, i+1);
	}
